...
add_action( 'widgets_init', 'tv9_custom_navigation_load_widgets' );

function tv9_custom_navigation_load_widgets() {
	register_widget( 'tv9_custom_navigation_widget' );
}

class tv9_custom_navigation_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'tv9_custom_navigation_widget', 'description' => esc_html__('A widget that allows you to either display posts in a row.', 'tv9-news') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 50, 'height' => 50, 'id_base' => 'tv9_custom_navigation_widget' );

		/* Create the widget. */
		parent::__construct( 'tv9_custom_navigation_widget', esc_html__('Custom Navigation', 'veegam'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) 
	{
		...
		$page_name_string = $instance['page_name_string'];
			...
			$args  = array( 
			  ...
			  'custom_data'  => array
						 (
							'size' => 'large',
							'is-active' => true,
							'request_data' => isset($wp->query_vars) ? $wp->query_vars : array(),
							'condition_arr' => isset($page_name_string) ? $page_name_string : array(),
						 ),
	...
	function update( $new_instance, $old_instance ) 
	{
		...
		$instance['page_name_string'] = $new_instance['page_name_string'];
		//$instance['post_type'] = strip_tags( $new_instance['post_type'] );
		$instance['showhide'] = strip_tags( $new_instance['showhide'] );
		
		
		//print_r($instance['page_name_string']);
		//die;
		return $instance;
	}
	...
	function form( $instance ) 
	{
		...
		for($i=0;$i<count($templatePartsFiles);$i++)
		{
			...
			if (strpos($dirName, 'page-navigation') !== false) 
			{
	...
	?>
        ...
        <?php 
			$page_name_string = array(
									'all-pages',
									'all-cricket-pages',
									'all-cricket-t20-pages',
									'all-cricket-ipl-pages',
									'all-election-pages',
									'all-gujarat-election-pages',
								);
			$option_name_1 = '';
			
			//print_r($instance['page_name_string']);
			for($i=0;$i<count($page_name_string);$i++)
			{
				if(isset($instance['page_name_string']) && in_array($page_name_string[$i],$instance['page_name_string']))
				{
					$option_name_1 .= '<option value="'.$page_name_string[$i].'" selected="selected">'.$page_name_string[$i].'</option>';
				}
				else
				{
					$option_name_1 .= '<option value="'.$page_name_string[$i].'">'.$page_name_string[$i].'</option>';
				}
			} 
		?>
        <p>
			<label for="<?php echo $this->get_field_id( 'page_name_string' ); ?>">Select Page Names:</label>
			<select id="<?php echo $this->get_field_id('page_name_string'); ?>" name="<?php echo $this->get_field_name('page_name_string'); ?>[]" style="width:100%;" multiple="multiple">
               <?php echo $option_name_1;?>
            </select>
           <?php /*?> <span>Select Page Name (Single or Multiple) To show custom Navigation.</span><?php */?>
        </p>
